...
import { LogIn, Mail, Lock, Sun, Moon, UserCircle2, MessageSquare, ArrowRight, Sparkles } from 'lucide-react';
          ...
          <div className="relative z-10 flex flex-col items-center animate-bounce-in">
            ...
            <div className={`relative bg-white text-indigo-700 font-extrabold px-6 py-4 rounded-2xl shadow-xl transition-all duration-500 ${isSuccess ? 'scale-110 bg-green-400 text-white' : 'hover:-translate-y-2'} mb-6`}>
              <div className="text-center text-lg whitespace-nowrap">
                {isSuccess ? "🎉 Thanks for Logging In!" : loading ? "Connecting..." : "👋 Welcome Back!"}
              </div>
              <div className="text-center text-xs opacity-80 mt-1">
                {isSuccess ? "Redirecting smoothly..." : "Sign in to continue."}
              </div>
              {/* Bubble Tail */}
          ...
          <div className="w-full space-y-8 animate-fade-in">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <Link to="/register" className="font-bold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 border-b-2 border-transparent hover:border-indigo-500 transition-colors">
                  Create one now
                </Link>
              </p>
            </div>
                ...
                >
                  <span className="relative z-10 flex items-center gap-2">
                    {loading && !isSuccess ? (
                      ...
                      <LogIn className="h-5 w-5 transform group-hover:scale-110 transition-transform" />
                    )}
                    <span>{isSuccess ? 'Success!' : loading ? 'Signing in...' : 'Sign In Securely'}</span>
                  </span>
